...
#vehicle = connect("/dev/ttyACM1", baud=57600, wait_ready=True)
...
def get_vehicle_state():
    ...
    #print(" Rangefinder: %s" % vehicle.rangefinder)("--- %s seconds ---" % (time.time() - start_time))
...
def get_vehicle_state_simple():
    ...
    print(" ***Velocity: %s" % vehicle.velocity)
    print(" Heading: %s" % vehicle.heading)
while vehicle.location.global_relative_frame.alt<=4.85:
    print " not reach.Altitude: ", vehicle.location.global_relative_frame.alt
    time.sleep(0.5)
print "Reached target altitude. Set velocity once"
set_velocity_body(vehicle, 0.1, 0.0, 0.0)
